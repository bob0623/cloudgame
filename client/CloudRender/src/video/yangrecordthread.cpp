...
YangRecordThread::YangRecordThread()
{
   ...
   // memset(m_playBuffer+m_playWidth*m_playHeight,128,(m_playWidth*m_playHeight)>>1);
    ...
    //m_recodeWriter = new AppTestFileWriter("play.yuv");
}
...
void YangRecordThread::render(){
    if(m_isRender&&m_syn){
        ...
        if(t_vb&&m_video){
            int width = m_syn->width(m_syn->session);
            int height = m_syn->height(m_syn->session);
            //m_recodeWriter->write((char*)t_vb, width*height*3/2);
            qDebug() << "YangRecordThread::render, getVideoRef, nb:" << m_frame.nb << playCount++;
