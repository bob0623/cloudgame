...
// TestFileWriter testwriter("player.264");
...
void YangRtcReceive::receiveAudio(YangFrame *audioFrame) {
    ...
    // qDebug() << "receiveAudio audioFrame pts: " << audioFrame->pts;
...
void YangRtcReceive::receiveVideo(YangFrame *videoFrame) {
    if(videoFrame==NULL||videoFrame->payload==NULL) return;
     qDebug()<<"receiveVideo videoFrame nb: "<<videoFrame->nb<<",frametype:"<<videoFrame->frametype << count++;
    // testwriter.write((char*)videoFrame->payload, videoFrame->nb);
...
int32_t YangRtcReceive::init(int32_t puid, char* localIp,
    ...
    qDebug()<<"YangRtcReceive::init: localIP = "<<localIp << ", port:"<<pport;
	...
	strcpy_s(m_recv->peer.streamconfig.remoteIp,server);
	m_recv->peer.streamconfig.remotePort = pport;
	strcpy_s(m_recv->peer.streamconfig.app,app);
	strcpy_s(m_recv->peer.streamconfig.stream,stream);
    ...
    qDebug()<<"create peerconnection: "<<m_recv->peer.streamconfig.remoteIp<<m_recv->peer.streamconfig.remotePort<<m_recv->peer.streamconfig.localPort;
