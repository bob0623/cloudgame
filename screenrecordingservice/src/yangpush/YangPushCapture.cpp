...
int32_t YangPushCapture::initVideo(int videostate){
	if(m_out_videoBuffer==NULL) m_out_videoBuffer = new YangVideoBuffer(m_context->avinfo.video.bitDepth==8?1:2);
    // if(m_screen_out_videoBuffer==NULL) m_screen_out_videoBuffer = new YangVideoBuffer(m_context->avinfo.video.bitDepth==8?1:2);
	...
	if (m_videoCapture == NULL) {
        ...
        // m_screen_out_videoBuffer->init(m_context->avinfo.video.width,m_context->avinfo.video.height,m_context->avinfo.video.videoEncoderFormat);
...
void YangPushCapture::startVideoCapture(){
    yang_trace(" YangPushCapture::startVideoCapture, open camera and callback function BufferCB will be called. \n");
		if(m_videoCapture&&!m_videoCapture->m_isStart) m_videoCapture->start();
}
void YangPushCapture::startScreenCapture(){
    qDebug()<<" YangPushCapture::startScreenCapture";
    if(m_videoCapture&&!m_videoCapture->m_isStart) m_videoCapture->start();
}
...
void YangPushCapture::startCamera() {
    initVideo(Yang_VideoSrc_Camera);
	startVideoCapture();
}
