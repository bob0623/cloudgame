...
int32_t YangPushPublish::startVideoCapture() {
	...
	if (m_capture == NULL)		m_capture=new YangPushCapture(m_context);
	int32_t err=Yang_Ok;
    if((err=m_capture->initVideo(Yang_VideoSrc_Camera))!=Yang_Ok) return yang_error_wrap(err,"init videoCapture fail");
...
int32_t YangPushPublish::startScreenShare(int videostate)
{
    if (isStartScreenCapture == 1)	return Yang_Ok;
    if (m_capture == NULL)		m_capture=new YangPushCapture(m_context);
    int32_t err=Yang_Ok;
    if((err=m_capture->initVideo(videostate))!=Yang_Ok) return yang_error_wrap(err,"init videoCapture fail");
    m_capture->startScreenCapture();
    isStartScreenCapture = 1;
    return err;
}

void YangPushPublish::stopScreenShare()
{
    if (m_capture ) m_capture->stopAll();
}
